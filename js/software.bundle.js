 ...
 * Show toast notification
 */
function showToastNotification(title, body, type, isHtml = true) {
 ...
 */
async function validateNewSoftwareForm() {
    const form = document.querySelector('.new-software-form');
    if (!form)
        return false;
    ...
    for (const input of form.querySelectorAll("input[type='url']")) {
        ...
        const feedbackLink = input.parentElement?.querySelector('.valid-feedback a');
        if (feedbackLink) {
            feedbackLink.href = input.value;
        }
    }
    // Parse BibTeX field
    const bibtexField = form.querySelector('#new-software-bibtex');
    if (bibtexField) {
        ...
        if (bibtexText === '') {
            ...
            if (feedbackEl)
                feedbackEl.innerHTML = 'No BibTeX provided.';
        }
        else {
    ...
    for (const select of form.querySelectorAll('#new-software-category, #new-software-language')) {
        const selected = Array.from(select.selectedOptions).map(o => o.value);
        const selectNewInput = select.nextElementSibling;
        ...
        if (selected.length === 0) {
            ...
            if (invalidFeedback)
                invalidFeedback.innerHTML = 'No value selected.';
        }
            ...
            if (invalidFeedback)
                invalidFeedback.innerHTML = 'New value not entered.';
        }
        else {
    ...
    // Validate keywords
    const keywordsField = form.querySelector('#new-software-keywords');
    if (keywordsField) {
    ...
    // Validate Zenodo DOI
    const doiInput = form.querySelector('#new-software-doi');
    if (doiInput) {
        try {
